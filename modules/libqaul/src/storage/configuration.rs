...
impl Default for Internet {
    fn default() -> Self {
        ...
        // /ip6/fe80::546f:f3ff:fedb:aac/udp/9229/quic-v1
        // /ip4/158.255.0.159/udp/9229/quic-v1
        Internet {
            active: true,
            peers: vec![InternetPeer {
                address: String::from("/ip4/158.255.0.159/udp/9229/quic-v1"),
                name: String::from("COICT"),
                enabled: true,
            }],
            do_listen: false,
...
/// Profile information for setting node profile
#[derive(Debug, Deserialize, Clone, Serialize)]
...
// In modules/libqaul/src/storage/configuration.rs
// Replace the init() function around line 273:
...
pub fn init() {
    ...
    {
        Err(e) => {
            log::error!("no configuration file found ({}), creating default one.", e);
            let default_config = Configuration::default();
            // Save the default config immediately
            CONFIG.set(RwLock::new(default_config.clone()));
            Configuration::save();
            default_config
        }
        Ok(c) => {
            match c.try_deserialize::<Configuration>() {
                Ok(config) => config,
                Err(e) => {
                    log::error!("Failed to deserialize config ({}), using default and backing up old config", e);
    ...
    /// Set complete node profile by updating the first user account
    /// This updates the configuration file and saves it
    pub fn set_node_profile(profile: UserProfile) {
        log::info!(
            "set_node_profile called with name: {}, college: {}",
            profile.name,
            profile.college
        );
        ...
        {
            let mut config = CONFIG.get().write().unwrap();
            if let Some(first_user) = config.user_accounts.first_mut() {
                log::info!("Updating config for user: {}", first_user.id);

                // Update all profile fields in config
                first_user.name = profile.name.clone();
                first_user.college = Some(profile.college.clone());
                first_user.reg_no = Some(profile.reg_no.clone());
                first_user.profile_pic = Some(profile.profile.clone());
                first_user.about = Some(profile.about.clone());
            } else {
                // If no user account exists, create one with the profile
                log::warn!("No user account found, creating new one");
                config.user_accounts.push(UserAccount {
                    name: profile.name.clone(),
                    id: String::from(""),
                    keys: String::from(""),
                    profile_pic: Some(profile.profile.clone()),
                    about: Some(profile.about.clone()),
                    reg_no: Some(profile.reg_no.clone()),
                    college: Some(profile.college.clone()),
                    storage: StorageOptions::default(),
                    active: true,
                });
            }

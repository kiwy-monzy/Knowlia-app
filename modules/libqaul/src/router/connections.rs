...
impl ConnectionTable {
    ...
    pub fn add_local_user(user_id: PeerId) {
        ...
        if routing_table.table.contains_key(&user_q8id) {
            // User already exists, just update their online time
            if let Some(entry) = routing_table.table.get_mut(&user_q8id) {
                let now_ts = Timestamp::get_timestamp() + 3000;
                entry.online_time = now_ts;
                entry.pgid_update = now_ts;
                entry.pgid_update_hc = 1;
            }
            return;
        }
        ...
        println!(
            "ROUTER TABLE: Added local user {} to LOCAL routing table",
            user_id.to_base58()
        );
    }

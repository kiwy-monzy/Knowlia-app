...
impl Neighbours {
    ...
    pub fn is_neighbour(node_id: &PeerId) -> ConnectionModule {
        log::debug!("Checking if peer {} is a neighbour", node_id.to_base58());
        
        // check if neighbour is in Lan table
        {
            let lan = LAN.get().read().unwrap();
            if lan.nodes.contains_key(node_id) {
                log::debug!("Peer {} found in LAN table", node_id.to_base58());
        ...
        // check if neighbour exists in Internet table
        {
            ...
            if internet.nodes.contains_key(node_id) {
                log::debug!("Peer {} found in Internet table", node_id.to_base58());
        ...
        // check if neighbour exists in BLE table
        {
            ...
            if ble.nodes.contains_key(node_id) {
                log::debug!("Peer {} found in BLE table", node_id.to_base58());
                return ConnectionModule::Ble;
            }
        }

        log::debug!("Peer {} is not a neighbour in any table", node_id.to_base58());
    ...
    /// Get a list of all connected neighbours from all modules
    pub fn get_all_nodes() -> Vec<PeerId> {
        let mut nodes: Vec<PeerId> = Vec::new();

        // get state
        {
            let lan = LAN.get().read().unwrap();
            for (id, _val) in lan.nodes.iter() {
                nodes.push(id.clone());
            }
        }
        {
            let internet = INTERNET.get().read().unwrap();
            for (id, _val) in internet.nodes.iter() {
    ...
    /// Get a list of all neighbours that are only connected via LAN module
    ///
    /// This function mirrors get_ble_only_nodes but for LAN. A node is
    /// considered "LAN-only" if it is present in the LAN neighbours table
    /// and not present in BLE or INTERNET neighbours tables.
    pub fn get_lan_only_nodes() -> Vec<PeerId> {
        let mut nodes: Vec<PeerId> = Vec::new();
        ...
        if lan.nodes.len() > 0 {
            ...
            for (peer_id, _) in lan.nodes.iter() {
                // node is not listed in BLE and not listed in INTERNET
                if !ble.nodes.contains_key(peer_id) && !internet.nodes.contains_key(peer_id) {
                    nodes.push(peer_id.clone());
                }
            }
        }
    ...
    /// Get a list of all neighbours that are only connected via INTERNET module
    ///
    /// This function mirrors get_ble_only_nodes but for INTERNET. A node is
    /// considered "INTERNET-only" if it is present in the INTERNET neighbours
    /// table and not present in LAN or BLE neighbours tables.
    pub fn get_internet_only_nodes() -> Vec<PeerId> {
        let mut nodes: Vec<PeerId> = Vec::new();
        ...
        // check if we have nodes listed
        if internet.nodes.len() > 0 {
            let lan = LAN.get().read().unwrap();
            let ble = BLE.get().read().unwrap();

            for (peer_id, _) in internet.nodes.iter() {
                // node is not listed in LAN and not listed in BLE
                if !lan.nodes.contains_key(peer_id) && !ble.nodes.contains_key(peer_id) {
                    nodes.push(peer_id.clone());
                }
            }
        }
    ...
    /// Get all neighbour peer IDs from all connection modules
    pub fn get_all_neighbours() -> Vec<PeerId> {
        let mut all_neighbours = Vec::new();

        // Collect from Internet neighbours
        let internet_neighbours = INTERNET.get().read().unwrap();
        all_neighbours.extend(internet_neighbours.nodes.keys().copied());

        // Collect from LAN neighbours
        let lan_neighbours = LAN.get().read().unwrap();
        all_neighbours.extend(lan_neighbours.nodes.keys().copied());

        // Collect from BLE neighbours
        let ble_neighbours = BLE.get().read().unwrap();
        all_neighbours.extend(ble_neighbours.nodes.keys().copied());

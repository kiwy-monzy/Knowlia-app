...
impl RouterInfo {
    ...
    pub fn received(received: QaulInfoReceived) {
        // validate received data before decoding
        if received.data.is_empty() {
            log::warn!(
                "Received empty router info data from {:?}",
        ...
        log::debug!(
            "Received router info data: {} bytes from {:?}",
            received.data.len(),
            received.received_from
        );

        // Check minimum expected size for RouterInfoContainer
        // RouterInfoContainer has at least: signature (bytes) + message (bytes)
        // Each bytes field has at least a length prefix, so minimum size is > 0
        if received.data.len() < 4 {
            log::warn!(
                "Received router info data too small ({} bytes) from {:?}",
                received.data.len(),
                received.received_from
            );
            return;
        ...
        match decoding_result {
            Ok(container) => {
                // validate container message before decoding
                if container.message.is_empty() {
                    log::warn!(
                        "Received router info container with empty message from {:?}",
                        received.received_from
                    );
                    return;
                }
                ...
                match message_result {
                    Ok(content) => {
                        // validate content before decoding inner messages
                        if content.content.is_empty() {
                            log::warn!(
                                "Received router info content with empty content from {:?}",
                                received.received_from
                            );
                            return;
                        }
                        ...
                        match router_net_proto::RouterInfoModule::try_from(
                            ...
                            Ok(router_net_proto::RouterInfoModule::UserUpdate) => {
                                // Process user update using the proper function from user_update module
                                super::user_update::process_user_update(
                                    content.content,
                                    received.received_from,
                                );
                            }
                    ...
                    Err(msg) => {
                        log::error!(
                            "RouterInfoContent decode error from {:?}: {} (message size: {} bytes, first 32 bytes: {:?})",
                            received.received_from,
                            msg,
                            container.message.len(),
                            container.message.get(..32).unwrap_or(&[])
                        );
                    }
                }
            }
            Err(msg) => {
                log::error!(
                    "RouterInfoContainer decode error from {:?}: {} (data size: {} bytes, first 32 bytes: {:?})",
                    received.received_from,
                    msg,
                    received.data.len(),
                    received.data.get(..32).unwrap_or(&[])
                );
            }
        }
    }

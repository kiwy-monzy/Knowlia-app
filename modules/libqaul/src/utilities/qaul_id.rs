...
impl QaulId {
    ...
    /// Convert a base58 string to a qaul ID (PeerId)
    ///
    /// This is the reverse operation of PeerId::to_base58().
    /// Returns an error if the string is not a valid base58 encoded PeerId.
    pub fn from_base58(base58_str: &str) -> Result<PeerId, String> {
        match PeerId::from_str(base58_str) {
            Ok(peer_id) => Ok(peer_id),
            Err(e) => Err(format!("Failed to parse base58 string as PeerId: {}", e)),
        }
    }
}

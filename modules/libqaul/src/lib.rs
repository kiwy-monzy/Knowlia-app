...
pub async fn start(storage_path: String, def_config: Option<BTreeMap<String, String>>) -> () {
    ...
    // initialize active users in router (after router is ready)
    crate::node::user_accounts::UserAccounts::init_active_users_in_router();

    // refresh user names from routing table after initialization
    router::users::Users::refresh_user_names();
    ...
    // log routing table every second
    let mut routing_table_log_ticker = Ticker::new(Duration::from_millis(1000));

    // check for user config updates every 5 seconds
    let mut user_update_ticker = Ticker::new(Duration::from_millis(5000));
    ...
    loop {
        let evt = {
            ...
            let routing_table_log_fut = routing_table_log_ticker.next().fuse();
            let user_update_fut = user_update_ticker.next().fuse();
            ...
            select! {
                ...
                _routing_table_log_event = routing_table_log_fut => Some(EventType::RoutingTableLog),
                _user_update_event = user_update_fut => Some(EventType::UserUpdate),
            }
        };
        ...
        if let Some(event) = evt {
            match event {
                ...
                EventType::RoutingTableLog => {
                    // log routing table every second
                    router::table::RoutingTable::log_routing_table();
                }
                EventType::UserUpdate => {
                    // check for user configuration changes and send updates
                    router::user_update::check_config_changes();
                    // process queued user update requests and send them
                    let user_update_requests =
                        router::user_update::UserUpdateSendRequester::process_requests();

                    for request in user_update_requests {
                        let connection_module = crate::router::neighbours::Neighbours::is_neighbour(
                            &request.neighbour_id,
                        );
                        match connection_module {
                            ConnectionModule::Lan => lan
                                ...
                                .behaviour_mut()
                                .qaul_info
                                .send_qaul_info_message(request.neighbour_id, request.data),
                            ConnectionModule::Internet => internet
                                .swarm
                                .behaviour_mut()
                                .qaul_info
                                .send_qaul_info_message(request.neighbour_id, request.data),
                            ConnectionModule::Ble => {
                                crate::connections::ble::Ble::send_routing_info(
                                    request.neighbour_id,
                                    request.data,
                                );
                            }

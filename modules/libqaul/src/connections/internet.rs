...
impl Internet {
    ...
    /// Get peer information for a given peer ID
    /// 
    /// Returns a tuple of (address, name, online_status) for the peer
    pub fn get_peer_info(peer_id: &PeerId) -> (Option<String>, Option<String>, bool) {
        // Get connections to find address
        let connections = INTERNETCONNECTIONS.get().read().unwrap();
        
        // Find address for this peer_id
        let mut address = None;
        for (addr, id) in connections.iter() {
            if id == peer_id {
        ...
        // For now, assume peer is online if it's in the connections table
        // TODO: Implement proper online status checking when swarm access is available

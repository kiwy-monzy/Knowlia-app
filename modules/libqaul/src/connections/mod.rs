...
impl Connections {
    ...
    /// Send user data to a specific peer via the appropriate connection module
    ///
    /// This function automatically determines the best connection module to use
    /// for the specified peer and handles the data transmission accordingly.
    ///
    /// # Arguments
    /// * `peer_id` - The target peer to send data to
    /// * `data` - The user data to send (already encoded)
    ///
    /// # Connection Module Behavior
    /// - **LAN**: Queues data for transmission via LAN swarm
    /// - **Internet**: Queues data for transmission via Internet swarm  
    /// - **BLE**: Sends immediately via BLE module
    /// - **Local**: No action needed for local users
    /// - **None**: Logs error if no connection available
    pub fn send_user_data(peer_id: libp2p::PeerId, data: Vec<u8>) {
        let connection_module = crate::router::neighbours::Neighbours::is_neighbour(&peer_id);

        match connection_module {
            ConnectionModule::Lan => {
                // Queue for LAN transmission - will be handled by the main event loop
                crate::router::user_update::UserUpdateSendRequester::queue_user_update(
                    peer_id, data,
                );
                log::debug!("Queued user data for LAN transmission to {:?}", peer_id);
            }
            ConnectionModule::Internet => {
                // Queue for Internet transmission - will be handled by the main event loop
                crate::router::user_update::UserUpdateSendRequester::queue_user_update(
                    peer_id, data,
                );
                log::debug!(
                    "Queued user data for Internet transmission to {:?}",
                    peer_id
                );
            }
            ConnectionModule::Ble => {
                // BLE has its own send function
                Ble::send_routing_info(peer_id, data);
                log::debug!("Sent user data via BLE to {:?}", peer_id);
            }
            ConnectionModule::Local => {}
            ConnectionModule::None => {
                log::error!(
                    "Cannot send user data to {:?} - no connection module available",
                    peer_id
                );
            }
        }
    ...
    /// Send user data to all neighbours
    pub fn send_user_data_to_all(data: Vec<u8>) {
        let neighbours = crate::router::neighbours::Neighbours::get_all_neighbours();
        let neighbour_count = neighbours.len();

        for neighbour_id in neighbours {
            Self::send_user_data(neighbour_id, data.clone());
        }

        log::debug!("Queued user data for {} neighbours", neighbour_count);
    }
}

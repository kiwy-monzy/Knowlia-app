...
/// Global event emitter for Tauri events
static mut EVENT_EMITTER: Option<Arc<dyn Fn(String, serde_json::Value) + Send + Sync>> = None;

/// Set the global event emitter function
/// This should be called from the Tauri layer to provide event emission
pub fn set_event_emitter(emitter: Arc<dyn Fn(String, serde_json::Value) + Send + Sync>) {
    unsafe {
        EVENT_EMITTER = Some(emitter);
    }
}

/// Emit a Tauri event if the emitter is set
pub fn emit_tauri_event(event_name: &str, payload: serde_json::Value) {
    unsafe {
        if let Some(ref emitter) = EVENT_EMITTER {
            emitter(event_name.to_string(), payload);
        }
    }
}

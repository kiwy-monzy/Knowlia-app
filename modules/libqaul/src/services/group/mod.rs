...
pub use manage::GroupManage as Manage;
...
// Re-export commonly used protobuf types for easier access
pub use proto_rpc::GroupMember as ProtoGroupMember;
pub mod proto_net {
        ...
        match proto_rpc::Group::decode(&data[..]) {
            Ok(group) => {
                match group.message {
                    ...
                    Some(proto_rpc::group::Message::GroupDeleteRequest(delete_req)) => {
                        let mut status = true;
                        let mut message: String = "".to_string();

                        if let Err(err) = Manage::delete_group(&my_user_id, &delete_req.group_id) {
                            status = false;
                            message = err.clone();
                        ...
                        let proto_message = proto_rpc::Group {
                            message: Some(proto_rpc::group::Message::GroupDeleteResponse(
                                proto_rpc::GroupDeleteResponse {
                                    group_id: delete_req.group_id.clone(),
                                    result: Some(proto_rpc::GroupResult { status, message }),
                                },
                            )),
                        };

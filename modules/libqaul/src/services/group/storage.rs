...
impl GroupStorage {
    ...
    pub fn group_get_unread_count(account_id: PeerId, group_id: Vec<u8>) -> u32 {
        log::debug!("group_get_unread_count");

        if let Some(group) = Self::get_group(account_id, group_id) {
            group.unread_messages
        } else {
            log::error!("group_get_unread_count group not found");
            0
        }
    }

    /// Get Total Unread Message Count Across All Groups
    pub fn group_get_total_unread_count(account_id: PeerId) -> u32 {
        log::debug!("group_get_total_unread_count");
        ...
        for res in db_ref.groups.iter() {
            if let Ok((_, group_bytes)) = res {
                if let Ok(group) = bincode::deserialize::<Group>(&group_bytes) {
                    total_unread += group.unread_messages;
                }
            }
        }
    ...
    /// Remove a group from the data base
    pub fn remove_group(account_id: PeerId, group_id: &Vec<u8>) {
        // get DB ref
        let db_ref = Self::get_db_ref(account_id);

        // remove group from data base
        if let Err(e) = db_ref.groups.remove(group_id) {
            log::error!("Error removing group from data base: {}", e);
        }
        
        // remove any pending group invite for this group
        if let Err(e) = db_ref.invited.remove(group_id) {
            log::error!("Error removing group invite from data base: {}", e);
        }
        
        // remove all chat messages associated with this group
        crate::services::chat::ChatStorage::delete_all_group_messages(&account_id, group_id);
        
        // flush groups tree to disk
        if let Err(e) = db_ref.groups.flush() {
            log::error!("Error groups flush: {}", e);
        }
        
        // flush invited tree to disk
        if let Err(e) = db_ref.invited.flush() {
            log::error!("Error invited flush: {}", e);
        }
    }

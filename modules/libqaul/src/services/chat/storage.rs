...
impl ChatStorage {
    ...
    /// delete messages by ids (alias for remove_messages)
    pub fn delete_messages(user_id: &PeerId, message_ids: &Vec<Vec<u8>>) {
        Self::remove_messages(user_id, message_ids);
    }

    /// delete all messages for a specific group
    pub fn delete_all_group_messages(user_id: &PeerId, group_id: &Vec<u8>) {
        let db_ref = Self::get_db_ref(user_id.clone());
        
        // create message keys range for this group
        let (first_key, last_key) = Self::get_db_key_range(group_id);
        
        // collect all message keys to delete
        ...
        {
            match res {
                Ok((key, _message_bytes)) => {
                    keys_to_delete.push(key.to_vec());
                }
                Err(e) => {
                    log::error!("delete_all_group_messages error collecting keys: {}", e);
                }
            }
        }
        ...
        for key in keys_to_delete {
            if let Err(_e) = db_ref.messages.remove(&key) {
                log::error!("delete_all_group_messages error removing message key!");
            }
        }
        
        // remove all message_ids for this group
        let mut message_ids_to_delete: Vec<Vec<u8>> = Vec::new();
        for res in db_ref.message_ids.iter() {
            match res {
                Ok((message_id, db_key)) => {
                    // check if this message_id belongs to the target group
                    if db_key.len() >= 16 && db_key[0..16] == *group_id.as_slice() {
                        message_ids_to_delete.push(message_id.to_vec());
                    }
                }
                Err(e) => {
                    log::error!("delete_all_group_messages error collecting message_ids: {}", e);
                }
            }
        }
        
        // delete all message_ids for this group
        for message_id in message_ids_to_delete {
            if let Err(_e) = db_ref.message_ids.remove(&message_id) {
                log::error!("delete_all_group_messages error removing message_id!");
            }
        }
        
        // flush changes to disk
        if let Err(_e) = db_ref.messages.flush() {
            log::error!("delete_all_group_messages error flushing messages!");
        }
        if let Err(_e) = db_ref.message_ids.flush() {
            log::error!("delete_all_group_messages error flushing message_ids!");
        }
        
        log::info!("Deleted all messages for group: {:?}", group_id);
    }

...
impl Decoder for VarintPrefixedCodec {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        if src.len() <= 10 {
            log::trace!("Insufficient data for varint decoding: {} bytes", src.len());
            return Ok(None);
        }
        ...
        let (message_length, remaining) = match unsigned_varint::decode::usize(src) {
            ...
            Err(unsigned_varint::decode::Error::Insufficient) => {
                log::trace!("Insufficient data for varint length: {} bytes", src.len());
                return Ok(None);
            }
            Err(err) => {
                log::debug!(
                    "Varint decode error: {} (first 32 bytes: {:?})",
                    err,
                    src.get(..32).unwrap_or(&[])
                );
                return Err(Error::new(ErrorKind::InvalidData, err));
            }
        };

        // check if package length is zero (invalid)
        if message_length == 0 {
            log::warn!("Received message with zero length, discarding");
            // Clear the varint to avoid infinite loop
        ...
        if src.len() < message_length + varint_length {
            log::trace!(
                "Incomplete message: need {} bytes, have {} bytes",
                message_length + varint_length,
                src.len()
            );
            return Ok(None);
        }
        ...
        let message_data = src.split_to(message_length).freeze().to_vec();
        log::trace!("Successfully decoded message: {} bytes", message_data.len());

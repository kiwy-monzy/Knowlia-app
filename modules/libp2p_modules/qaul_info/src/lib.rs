...
impl NetworkBehaviour for QaulInfo {
    ...
    fn on_connection_handler_event(
        ...
        let qaul_info_data = match event {
            ...
            Ok(InnerMessage::Received(event)) => {
                // Validate data before forwarding
                if event.data.is_empty() {
                    log::warn!("Received empty qaul_info_data from peer {}", peer_id);
                    return;
                }
                if event.data.len() < 4 {
                    log::warn!(
                        "Received qaul_info_data too small ({} bytes) from peer {}",
                        event.data.len(),
                        peer_id
                    );
                    return;
                ...
                log::debug!(
                    "Forwarding qaul_info_data: {} bytes from peer {}",
                    event.data.len(),
                    peer_id
                );
                event

        ...
        Box::pin(async move {
            ...
            let data: Vec<u8> = match incoming_data {
                Ok(data) => {
                    ...
                    if data.is_empty() {
                        log::warn!("Received empty qaul_info data from connection");
                    ...
                    if data.len() < 4 {
                        log::warn!(
                            "Received qaul_info data too small ({} bytes) - possible corruption",
                            data.len()
                        );
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Data too small"));
                    }
                    log::debug!("Received qaul_info data: {} bytes", data.len());
                    data
                }
                Err(err) => {
                    log::debug!("Failed to receive qaul_info data: {}", err);
                    return Err(err);
                }
            };

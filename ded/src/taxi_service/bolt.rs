...
use dotenvy;
use reqwest::header::{
    HeaderMap, HeaderValue, ACCEPT, ACCEPT_LANGUAGE, AUTHORIZATION, CACHE_CONTROL, CONNECTION,
    HOST, USER_AGENT,
};
...
pub fn get_coordinates_from_env() -> (LatLng, LatLng) {
    ...
    let lat_env = "-6.772355";
    let lng_env = "39.240061";

    let lat = lat_env.parse::<f64>().unwrap_or(-6.772355);
    let lng = lng_env.parse::<f64>().unwrap_or(39.240061);
...
pub async fn fetch_taxi_vehicles() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let _destinations = vec![destination];
    let destination = LatLng {
        lat: -6.7812789999999996,
        lng: 39.215254999999999,
    };
    let viewport = generate_viewport_with_padding(pickup, destination, 100.0);
    let auth_username = "uid_247899280".to_string();
    let device_uuid = "F4669DCE-9F61-480A-9C02-345F12A78C32".to_string();

    let token = base64::engine::general_purpose::STANDARD
        .encode(format!("{}:{}", auth_username, device_uuid));
    let url = format!(
    ...
    headers.insert(
        USER_AGENT,
        HeaderValue::from_static("Bolt/161195418 CFNetwork/1568.300.101 Darwin/24.2.0"),
    );
...
pub async fn start_verification(
    ...
    dotenvy::dotenv().ok();
    let _auth_username = "uid_247899280".to_string();
    let device_uuid = "F4669DCE-9F61-480A-9C02-345F12A78C32".to_string();
...
pub async fn get_location_suggestions(

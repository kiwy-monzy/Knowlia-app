...
use crate::str0m::util;
use tauri::{Manager, path::BaseDirectory};

fn load_existing_cert(app: &tauri::AppHandle) -> Result<(Vec<u8>, Vec<u8>), Box<dyn std::error::Error>> {
    // Try to load certificate files from resources first
    let cert_result = app.path().resolve("cer.pem", BaseDirectory::Resource);
    let key_result = app.path().resolve("key.pem", BaseDirectory::Resource);
    
    match (cert_result, key_result) {
        (Ok(cert_path), Ok(key_path)) => {
            // Try to read from resource paths
            match (app.fs().read(&cert_path), app.fs().read(&key_path)) {
                (Ok(cert_content), Ok(key_content)) => {
                    info!("Loaded certificate files from resources");
                    return Ok((cert_content, key_content));
                }
                _ => {
                    warn!("Failed to read certificate files from resources, trying embedded certificates");
                }
            }
        }
        _ => {
            warn!("Failed to resolve certificate resource paths, trying embedded certificates");
        }
    }
    
    // Fallback to embedded certificates
    let cert_content = include_str!("../../assets/cer.pem");
    let key_content = include_str!("../../assets/key.pem");
    
    info!("Loaded embedded certificate files");
    Ok((cert_content.as_bytes().to_vec(), key_content.as_bytes().to_vec()))
}
...
fn init_log() {
    ...
    if env::var("RUST_LOG").is_err() {
        env::set_var("RUST_LOG", "chat=info,str0m=info,init_log=debug");
...
pub fn main(app: &tauri::AppHandle) {
    init_log();

    let (certificate, private_key) = match load_existing_cert(app) {
        Ok((cert, key)) => {
            info!("Loaded existing certificate files");
            (cert, key)
        }
        Err(e) => {
            warn!("Failed to load certificate files: {}. Falling back to HTTP.", e);
            ...
            // Try different ports for HTTP as well
            let mut port = 3022;
            let server = loop {
                let tx_for_server = tx.clone();
                match Server::new(
                    &format!("0.0.0.0:{}", port),
                    move |request| web_request(request, addr, tx_for_server.clone()),
                ) {
                    Ok(server) => break server,
                    Err(e) => {
                        if let Some(io_err) = e.downcast_ref::<std::io::Error>() {
                            if io_err.kind() == std::io::ErrorKind::AddrInUse {
                                warn!("Port {} is in use, trying port {}", port, port + 1);
                                port += 1;
                                if port > 3030 {
                                    panic!("Could not find an available port between 3022-3030");
                                }
                                continue;
                            }
                        }
    ...
    // Spin up a UDP socket for RTC. All WebRTC traffic is going to be multiplexed over this single
    ...
    // Start HTTPS server with generated certificate, try different ports if needed
    let mut port = 3022;
    let server = loop {
        let tx_for_server = tx.clone();
...
fn web_request(request: &Request, addr: SocketAddr, tx: SyncSender<Rtc>) -> Response {
    ...
    // Add multiple ICE candidates for better connectivity
    // Add host candidate
    let host_candidate = Candidate::host(addr, "udp").expect("a host candidate");
    rtc.add_local_candidate(host_candidate);
    
    // Try to add srflx candidates (server reflexive)
    // Note: In a production environment, you would want to add STUN server candidates here
    info!("Added local ICE candidate: {}", addr);
    ...
    if let Err(e) = tx.send(rtc) {
        warn!("Failed to send Rtc instance to main loop: {:?}", e);
        warn!("Channel might be full or disconnected. This usually happens when the server is overloaded.");
        return Response::text("Server error: unable to join room. The server might be busy, please try again.").with_status_code(500);
    }
...
fn spawn_new_client(rx: &Receiver<Rtc>) -> Option<Client> {
    ...
    match rx.try_recv() {
        ...
        Err(TryRecvError::Disconnected) => {
            debug!("Receiver<Rtc> disconnected - no more incoming connections");
            None
        }
    }
}
...
impl Client {
    ...
    fn negotiate_if_needed(&mut self) -> bool {
        ...
        for track in &mut self.tracks_out {
            if let TrackOutState::ToOpen = track.state {
                if let Some(track_in) = track.track_in.upgrade() {
                    ...
                    let mid =
                        change.add_media(track_in.kind, Direction::SendOnly, Some(stream_id), None, None);
        ...
        if let Err(e) = channel.write(false, json.as_bytes()) {
            warn!("Failed to send answer to channel: {:?}", e);
            return false;
        }

...
export const useRTC = (options: UseRtcOptions = {}) => {
  ...
  // Get pending SDP offer for an incoming call
  const getPendingOffer = useCallback(async (groupId: string) => {
    if (!isInitialized) return null;
    
    try {
      return await rtcService.getPendingOffer(groupId);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to get pending offer');
      return null;
    }
  }, [isInitialized]);

  // Get SDP answer for an established call
  const getSdpAnswer = useCallback(async (groupId: string) => {
    if (!isInitialized) return null;
    
    try {
      return await rtcService.getSdpAnswer(groupId);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to get SDP answer');
      return null;
    }
  }, [isInitialized]);

  // Accept call with custom SDP answer
  const acceptCallWithAnswer = useCallback(async (groupId: string, sdpAnswer: string) => {
    if (!isInitialized) return;
    ...
    try {
      const session = await rtcService.acceptCallWithAnswer(groupId, sdpAnswer);
      await refreshSessions();
      return session;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to accept call with answer');
      throw err;
    } finally {
      setIsLoading(false);
    }
  ...
  // Start str0m WebRTC server
  const startStr0mServer = useCallback(async () => {
    try {
      await rtcService.startStr0mServer();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to start str0m server');
      throw err;
    }
  }, []);

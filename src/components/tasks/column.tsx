...
export default function Column({
  ...
  const headerColorClass = column.color || "bg-[#fafafa]"

  return (
    <div className="shrink-0 w-72 flex flex-col bg-[#fafafa] rounded-md shadow-sm">
      <div className={`p-3 flex justify-between items-center border-b rounded-t-md ${headerColorClass}`}>
        <h3 className="font-medium text-sm text-[#1a1a1a] flex items-center">
          {column.title}
      ...
      <Droppable droppableId={column.id}>
        {(provided) => (
          <div ref={provided.innerRef} {...provided.droppableProps} className="flex-1 p-2 overflow-y-auto">
            ...
            {isAddingTask ? (
              <div className="mt-2 p-3 bg-[#fafafa] rounded-md shadow-sm border border-gray-200">

...
mod proto {
    include!("../../../../../modules/libqaul/src/rpc/protobuf_generated/rust/qaul.rpc.user_accounts.rs");
}
...
impl UserAccounts {
    ...
    pub fn cli(command: &str) {
        match command {
            ...
            cmd if cmd.starts_with("create ") => {
                Self::parse_create_command(cmd.strip_prefix("create ").unwrap());
            }
            // unknown command
    ...
    /// Parse create command with optional fields
    fn parse_create_command(command: &str) {
        let parts: Vec<&str> = command.split_whitespace().collect();

        if parts.is_empty() {
            log::error!("create command requires at least a name");
            return;
        }
        ...
        while i < parts.len() {
            match parts[i] {
                "--profile-pic" | "-p" => {
                    if i + 1 < parts.len() {
                        profile_pic = Some(parts[i + 1].to_string());
                        i += 2;
                    } else {
                        log::error!("--profile-pic requires a value");
                        return;
                    }
                }
                "--about" | "-a" => {
                    if i + 1 < parts.len() {
                        about = Some(parts[i + 1].to_string());
                        i += 2;
                    } else {
                        log::error!("--about requires a value");
                        return;
                    }
                }
                "--reg-no" | "-r" => {
                    if i + 1 < parts.len() {
                        reg_no = Some(parts[i + 1].to_string());
                        i += 2;
                    } else {
                        log::error!("--reg-no requires a value");
                        return;
                    }
                }
                "--college" | "-c" => {
                    if i + 1 < parts.len() {
                        college = Some(parts[i + 1].to_string());
                        i += 2;
                    } else {
                        log::error!("--college requires a value");
                        return;
                    }
                }
                _ => {
                    log::error!("unknown option: {}", parts[i]);
                    return;
                }
            }
        }

        Self::create_user_account(name, profile_pic, about, reg_no, college);
    }
            ...
            Ok(user_accounts) => {
                match user_accounts.message {
                    Some(proto::user_accounts::Message::DefaultUserAccount(
                        ...
                        if proto_defaultuseraccount.user_account_exists {
                            if let Some(my_user_account) = proto_defaultuseraccount.my_user_account
                            {
                                ...
                                if let Some(ref profile_pic) = my_user_account.profile_pic {
                                    println!("    profile picture: {}", profile_pic);
                                }
                                if let Some(ref about) = my_user_account.about {
                                    println!("    about: {}", about);
                                }
                                if let Some(ref reg_no) = my_user_account.reg_no {
                            ...
                            println!("");
                            println!("    account create {{Your User Name}} [--profile-pic|-p URL] [--about|-a \"About text\"] [--reg-no|-r \"Registration number\"] [--college|-c \"College\"]");
                            println!("");
                    ...
                    Some(proto::user_accounts::Message::MyUserAccount(proto_myuseraccount)) => {
                        ...
                        // print optional fields if they exist
                        if let Some(ref profile_pic) = proto_myuseraccount.profile_pic {
                            println!("    profile picture: {}", profile_pic);
                        }
                        if let Some(ref about) = proto_myuseraccount.about {
                            println!("    about: {}", about);
                        }
                        if let Some(ref reg_no) = proto_myuseraccount.reg_no {
                            println!("    registration number: {}", reg_no);
                        }
